...
class Thread(Base, HasPublicID, HasRevisions):
    ...
    def also_set_tag(self, key, folderitem, is_remove):
        ...
        # removed.
        # TODO(emfree): Currently, this lazy generation of tag objects means
        # that we don't expose folders through the tags API if they exist on
        # the backend, but don't have synced messages associated with them.
    ...
    def remove_tag(self, tag, execute_action=False):
        ...
        tagitem = object_session(self).query(TagItem). \
            filter(TagItem.thread == self,
                   TagItem.tag == tag).one()
        tagitem.action_pending = True
...
class Tag(Base, HasRevisions):
    ...
    def create_canonical_tags(cls, namespace, db_session):
        ...
        exist."""
        for canonical_name in cls.RESERVED_TAG_NAMES:
            existing_tag = db_session.query(Tag).filter(
                Tag.namespace == namespace,
                Tag.public_id == canonical_name).first()
